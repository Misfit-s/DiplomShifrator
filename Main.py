...
while True:
    ...
    if event == sg.WINDOW_CLOSED:  # Указание завершения программы при нажатии на кнопку закрытия окна
        ...
        def PassEncrypt(passwrd):  # Функция шифрования мастер-ключа
            passwrd = passwrd.encode("utf-8")  # Кодирование мастер-ключа в кодировку utf-8
            ...
            conn.commit()  # Запись iv в базу данных
        ...
        def CheckDbExists():  # Функция проверки существования базы данных
            ...
            if table_exists:
                ...
                if dbInit == "OK":
                    ...
                    conn.commit()  # Очистка базы данных
                    cursor.execute("INSERT INTO password (salt, iv, ciphered_bytes) VALUES (NULL, NULL, NULL);")
                    conn.commit()  # заполение базы данных значениями NULL
            ...
            if table == "OK":
                ...
                conn.commit()  # Указание и выполнение запроса на генерацию таблицы и полей
                cursor.execute("INSERT INTO password (salt, iv, ciphered_bytes) VALUES (NULL, NULL, NULL);")
                conn.commit()
        ...
        def PassGen():  # Функция генерации мастер-ключа
            ...
            window['-PASS-'].update(
                "".join(map(str, GenPass))[:pass_length])  # Вывод сгенерированного мастер-ключа в поле ввода
